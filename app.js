(() => {
  ...
  function listeningView(test) {
    ...
    const parts = test.parts.map(part => {
      ...
      const transcript = part.transcript && !test.preferTranscriptSource ? `<details class="listening-transcript"><summary>Complete listening script</summary><div>${part.transcript.map(line => `<p>${clickable(line)}</p>`).join("")}</div></details>` : "";
      ...
      const transcriptSource = part.transcript ? `<section class="audio-script-source"><div class="audio-script-heading"><div><p class="eyebrow">Teacher-supplied transcript</p><h3>Audio script</h3></div><span>Click highlighted words to look them up</span></div><div class="audio-script-text">${part.transcript.map(line => `<p>${clickable(line)}</p>`).join("")}</div><p class="rights-note">${esc(test.transcriptNote)}</p></section>` : "";
      const sourceSheet = test.preferTranscriptSource ? transcriptSource : test.preferTextSource ? textSource : part.sourceImage ? `<details class="question-source"><summary>Original question sheet</summary><a href="${part.sourceImage}" target="_blank"><img src="${part.sourceImage}" alt="${esc(part.title)} original question sheet" loading="lazy"></a></details>` : `<div class="question-source text-source"><strong>Question source</strong><p>The teacher supplied this part as complete text.</p></div>`;
